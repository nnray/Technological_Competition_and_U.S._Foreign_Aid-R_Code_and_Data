...
GDP<-read.csv(here("Data", "GDP", "API_NY.GDP.PCAP.CD_DS2_en_csv_v2_4498542.csv"),skip = 4) #how tf to clean this?
```
...
Caid<-rename(Caid, china_constant2017usd=total_constant)
...
ITU<-rename(ITU, indicator_name=`Indicator name`)
ITU<-rename(ITU, total_internet=Value) #total percent of individuals using internet
ITU<- ITU %>%
  select(`country_name`,`year`,`total_internet`)
USaid<-rename(USaid, year=`Fiscal.Year`)
USaid<-rename(USaid, us_constant2021usd=`constant_amount`) #US aid variable, constant USD
USaid<-USaid %>% 
  select(`country_name`,`year`,`constant_disbursements`)
USaid<-transform(USaid, year=as.numeric(year))
data<- full_join(Caid, DSP, by = c("country_name","year")) %>%
  full_join(., ITU, by = c("country_name","year")) %>%
  full_join(., USaid, by = c("country_name","year")) %>%
  full_join(., VDem, by = c("country_name","year"))
```
```{r Analysis}
