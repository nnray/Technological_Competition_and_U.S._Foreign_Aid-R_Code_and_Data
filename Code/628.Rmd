...
#political variable?
pdim(data)
model1<-plm(log_usaid~v2smgovsmcenprc+log_caid+log_gdp+internet+v2x_polyarchy,index = c("country","year"),data=data)
pbgtest(model1)
c1<-vcovHC(model1,cluster="group")
coeftest(model1,vcov. = c1)
model2<-plm(log_usaid~v2smgovsmcenprc+log_caid+log_gdp+internet+v2x_polyarchy,index = c("country","year"),data=data,model="random",random.method = "walhus")
c2<-vcovHC(model2,cluster="group")
coeftest(model2,vcov. = c2)
```